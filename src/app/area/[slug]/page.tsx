...
import { getAreaMeta } from "@/lib/data";
import { AreaSlug } from "@/lib/types";
import { FilterBar } from "@/components/filters/FilterBar";
import { Badge } from "@/components/ui/Badge";
import { Breadcrumb } from "@/components/ui/Breadcrumb";
import { AreaMapLoader } from "@/components/areas/AreaMapLoader";
...
export default async function AreaPage({ params }: Props) {
  ...
  const areaMeta = getAreaMeta(slug as AreaSlug);
    ...
    <div>
      <section className="bg-gradient-to-b from-[var(--color-section-dark)] via-primary/95 to-neutral-50 py-14 sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-3 text-white/70 text-sm">{area.description}</p>
          ...
          <p className="mt-3 text-[10px] text-white/40 uppercase tracking-wider">
            Verified {LAST_VERIFIED}
          </p>

          <div className="mt-8 bg-white rounded-xl shadow-sm p-6 border border-neutral-100">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-display font-semibold text-neutral-900">Area overview</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
              {statItems.map((stat) => (
                <div
                  key={stat.label}
                  className="text-center bg-primary-muted/50 rounded-lg p-3 border border-neutral-100"
                >
                  <p className="text-xl font-display font-semibold text-primary">
                    {stat.value}
                  </p>
                  <p className="text-[9px] text-neutral-500 uppercase tracking-wider font-medium mt-0.5">
                    {stat.label}
                  </p>
                </div>
              ))}
            ...
            <div className="flex flex-wrap items-center gap-6 mt-5 pt-5 border-t border-neutral-200">
      ...
      {areaMeta && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 pb-4">
          <h2 className="font-display text-lg font-semibold text-neutral-900 mb-3">Map</h2>
          <AreaMapLoader area={areaMeta} providers={providers} />
        </section>
      )}

      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="mb-8 max-w-3xl">
          <h2 className="font-display text-lg font-semibold text-neutral-900 mb-3">
